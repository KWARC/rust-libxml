#!/bin/bash
# thanks to the original at: https://github.com/shepmaster/sxd-document/blob/master/deploy_gh_pages.sh
set -eux

if [[ "${TRAVIS_BRANCH}" != 'master' ]] || [[ "${TRAVIS_PULL_REQUEST}" = 'true' ]] || [[ "${TRAVIS_RUST_VERSION}" != 'stable' ]]; then
   exit 0
fi
