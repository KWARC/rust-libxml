...
fn xpath_result_number_correct() {
  ...
  let doc = doc_result.unwrap().into_readonly();
  let context = RoContext::new(&doc).unwrap();
...
fn findnodes_interfaces() {
  ...
  let p2_result = body_node.findnodes_readonly("p", &doc);
...
fn xpath_find_string_values() {
  ...
  assert!(doc_result.is_ok());
  let doc = doc_result.unwrap().into_readonly();
  let xpath = RoContext::new(&doc).unwrap();
  match doc.get_root_element() {
    Some(root) => {
      ...
      let node_ids_values = ids_test.findvalues_readonly(".//@xml:id", &doc);
